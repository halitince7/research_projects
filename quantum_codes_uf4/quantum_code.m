...
F  := "c:/users/halit/desktop/faikhepsi.txt";
F1 := "c:/users/halit/desktop/faikparam.txt"; 
  ...
  for C2 in Codes do
      d2 := MinimumDistance(C2);
           ...
           if (s eq 0) and (d ne 1) and (k ne 0) then
              ...
              Write(F,g1); Write(F,g2);
              Write(F,ShortG(n,g1)); Write(F,ShortG(n,g2));

              Write(F," Reciprocal-1,Reciprocal-2: ");
              Write(F,RecipG(g1)); Write(F,RecipG(g2));
              
              Write(F," RecipH-1,RecipH-2: ");
              Write(F,RecipH(n,g1)); Write(F,RecipH(n,g2));
